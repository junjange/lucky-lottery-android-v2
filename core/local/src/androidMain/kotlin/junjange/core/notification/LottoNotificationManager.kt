...
class LottoNotificationManager(
    ...
    fun sendLottoNotification() {
        val intent =
            Intent(context, config.mainActivityClass).apply {
        ...
        val largeIcon = BitmapFactory.decodeResource(context.resources, config.appIconRes)

        val notification =
            NotificationCompat
                .Builder(context, LOTTO_CHANNEL_ID)
                .setSmallIcon(config.appIconRes)
